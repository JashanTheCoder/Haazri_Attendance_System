...
if (!SKIP_DB) {
  ...
  console.log("⚠️ SKIP_DB enabled: database sync skipped");
}
...
app.post("/saveAttendance", async (req, res) => {
  ...
  try {
    ...
    for (const course of courses) {
      ...
      const [attendance, created] = await Attendance.findOrCreate({where: {UserId: user.id, courseName: course.courseName}, defaults: {delivered: course.delivered, attended: course.attended, percentage: course.percentage}}); if (!created) {await attendance.update({delivered: course.delivered, attended: course.attended, percentage: course.percentage}); } io.emit('attendanceUpdated', {
        username,
...
server.listen(PORT, () => {
  console.log(`🚀 Server + Socket.IO + GraphQL running on http://localhost:${PORT}`);
